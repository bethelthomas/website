...
{ 
$Sname = $_POST['sname'];
$RESULT = mysqli_query($mysqli,"SELECT count(*) FROM basicdetails WHERE  `Approve`='1' AND (`FirstName` LIKE '%$Sname%' or `LastName` LIKE '%$Sname%')");
$data=mysqli_fetch_assoc($RESULT);
echo "<br><h3>Number of Students with Name '$Sname'&nbsp:&nbsp";
echo $data['count(*)'];
echo "</h3>"; 
$sql = mysqli_query($mysqli,"SELECT * FROM basicdetails WHERE `Approve`='1' AND (`FirstName` LIKE '%$Sname%' or `LastName` LIKE '%$Sname%')");
...
?>
 ...
 <footer class="text-right">
            ...
            |  Developed by <a href="" target="_parent">Xavierites</a></p>
          </footer>         
        </div>
      </div>
    </div>    
