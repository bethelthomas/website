...
$mysqli = new mysqli('remotemysql.com','NG73FMUEBv','AOMDJxJRXe','NG73FMUEBv');
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($mysqli,"SELECT * FROM `placement`.`prilogin` WHERE `Username`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($mysqli,"UPDATE `placement`.`prilogin` SET `Password` = '$Password' WHERE `prilogin`.`Username` = '$Username'"))
					{
